...
def collate_fn_da(batch):
    source_imgs, source_labels,target_imgs ,target_labels,target_imgs_strong_aug= list(zip(*batch))
    samples = nested_tensor_from_tensor_list(source_imgs + target_imgs)  #合并

    #判断是否经过强增广处理
    if target_imgs_strong_aug[0] is not None:
        samples_strong_aug = nested_tensor_from_tensor_list(source_imgs + target_imgs_strong_aug)  #与弱增广保持相同处理，以保证维度统一，方便后续操作
    else:
